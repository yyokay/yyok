...
function CRUD(options) {
  const defaultOptions = {
    ...
    // 重置表单
    defaultForm: () => {},
    // 排序规则，默认 coder 降序， 支持多字段排序 ['coder,desc', 'createTime,asc']
    sort: ['coder,desc'],
    // 等待时间
    time: 50,
    // CRUD Method
  ...
  const methods = {
    ...
    toEdit(data) {
      ...
      crud.getDataStatus(data.coder).edit = CRUD.STATUS.PREPARED
     ...
     */
    toDelete(data) {
      crud.getDataStatus(data.coder).delete = CRUD.STATUS.PREPARED
    },
    /**
     * 取消删除
     * @param {*} data 数据项
    ...
    cancelDelete(data) {
      ...
      crud.getDataStatus(data.coder).delete = CRUD.STATUS.NORMAL
    ...
    cancelCU() {
      ...
      if (editStatus === CRUD.STATUS.PREPARED) {
        ...
        crud.getDataStatus(crud.form.coder).edit = CRUD.STATUS.NORMAL
      }
      crud.resetForm()
      if (addStatus === CRUD.STATUS.PREPARED) {
    ...
    doDelete(data) {
      ...
      if (data instanceof Array) {
        delAll = true
        data.forEach(val => {
          coders.push(val.coder)
        })
      } else {
        coders.push(data.coder)
        dataStatus = crud.getDataStatus(data.coder)
      }
      ...
      return crud.crudMethod.del(coders).then(() => {
        if (delAll) {
    ...
    /**
     * 获取数据状态
     * @param {Number | String} coder 数据项coder
     */
    getDataStatus(coder) {
      return crud.dataStatus[coder]
    },
    /**
     * 用于树形表格多选, 选中所有
     * @param selection
    ...
    selectChange(selection, row) {
      ...
      if (selection.find(val => { return val.coder === row.coder })) {
        if (row.children) {
