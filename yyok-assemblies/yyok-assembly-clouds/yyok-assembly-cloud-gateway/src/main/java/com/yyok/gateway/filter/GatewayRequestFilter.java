package com.yyok.gateway.filter;

import com.yyok.gateway.constant.Constant;
...
public class GatewayRequestFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        byte[] token = Base64Utils.encode((Constant.Security.TOKENVALUE.getVal()).getBytes());
        String[] headerValues = {new String(token)};
        ServerHttpRequest build = request.mutate().header(Constant.Security.TOKENHEADER.getVal(), headerValues).build();
